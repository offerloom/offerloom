...
const run = (values = {}) => ({ display_title: "Cloud catalogue · publish", created_at: "2026-09-24T01:30:00Z", status: "completed", conclusion: "success", ...values });
test("dispatches a publish run when today's schedule is missing; probes do not suppress it", async () => {
  ...
  assert.deepEqual(JSON.parse(calls[1].options.body), { ref: "main", inputs: { probe: false, force: false } });
});
