...
- Cloudflare's account had reached its five Cron Trigger free limit; the first scheduler deployment with four new schedules failed with code 10072 (it uploaded code but did not attach triggers). Deployed scheduler corrected to exactly two primary schedules, `30 1 * * *` and `30 11 * * *` UTC (07:00 and 17:00 IST), verified on Worker version `5c24e3ad-5aca-40e3-a211-6fec01cf3c2a`. Existing GitHub native backups remain at 08:00 and 18:00 IST. Window-specific retry limit is two attempts/day. Source alignment follow-up is PR #6.
- Local `npm test` passed build plus 59 tests; collector/scheduler focused tests passed 26/26; changed-file ESLint had zero errors (three existing image warnings); `git diff --check` passed. The live forced catalogue refresh and resulting product totals remain to be verified after this note.
